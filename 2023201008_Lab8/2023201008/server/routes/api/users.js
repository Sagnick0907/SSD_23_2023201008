...
// Create a route to delete data by quesid
router.delete('/delete/:quesid', (req, res) => {
    const quesid = req.params.quesid; // Get the quesid from the URL parameters
  
    // Use Mongoose to find and delete the document by quesid
    Qa.findOneAndRemove({ quesid: quesid })
      .then(deletedDoc => {
        if (!deletedDoc) {
...
router.post("/updateQuesAnswer", authenticateUser, (req, res) => {
    ...
    console.log(status);
    if (!quesid || !status) {
        return res.status(400).json({ error: "quesid and status are required. It should not be empty!" });
    }

    // Find the document by 'quesid' and update 'question' and 'answer'
    Qa.findOneAndUpdate({ quesid: quesid }, { status: status }, { new: true })
        .then(updatedDoc => {
            if (!updatedDoc) {
...
router.get("/allCompleteQuestions", async (req, res) => {
    try {
        // Fetch all documents from the "qa" collection with status: false
        const qaData = await Qa.find({ status: true });
        res.json(qaData);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: "Internal Server Error" });
...
router.get("/allIncompleteQuestions", async (req, res) => {
    try {
        // Fetch all documents from the "qa" collection with status: false
        const qaData = await Qa.find({ status: false });
        res.json(qaData);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: "Internal Server Error" });
